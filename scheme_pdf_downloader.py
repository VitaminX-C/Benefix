...
# ======================
# Reads CSV files that contain a "url" or "Scheme URL" column, 
# visits each URL, crawls the page (and, optionally, one level of linked pages 
# on the same site) looking for links to PDF files, and downloads every PDF 
# it finds into a local "Scheme_PDFs" folder.
#
# Usage
# -----
#     pip install requests beautifulsoup4 tqdm
#
#     # Run with defaults (automatically fetches student_schemes.csv and business_schemes.csv)
#     python scheme_pdf_downloader.py
#
#     # Options
#     python scheme_pdf_downloader.py --output Scheme_PDFs --depth 1 --workers 8
...
def read_scheme_rows(csv_paths):
    ...
    for path in csv_paths:
        with open(path, newline="", encoding="utf-8", errors="replace") as fh:
            ...
            for row in reader:
                url = (row.get("url") or row.get("Scheme URL") or "").strip()
                name = (row.get("scheme_name") or row.get("Scheme Name") or "").strip() or "Untitled Scheme"
...
def main():
    ...
    # Automatically default to these two files if nothing is typed in the terminal
    parser.add_argument("csv_files", nargs="*", default=["student_schemes.csv", "business_schemes.csv"], 
                        help="One or more CSV files (defaults to student_schemes.csv and business_schemes.csv)")
    ...
    # Filter out missing files so the script doesn't completely crash if one hasn't been generated yet
    valid_files = [f for f in args.csv_files if os.path.isfile(f)]
    
    if not valid_files:
        print(f"ERROR: Could not find any of the specified CSV files: {args.csv_files}", file=sys.stderr)
        sys.exit(1)
            
    rows = list(read_scheme_rows(valid_files))
    print(f"Loaded {len(rows)} unique scheme URLs from {len(valid_files)} CSV file(s).")
